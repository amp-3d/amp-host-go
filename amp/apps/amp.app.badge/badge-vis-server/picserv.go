...
func hello(w http.ResponseWriter, req *http.Request) {
	...
	canvas := svg.New(w)
	canvas.Start(width, height)
	canvas.Title("tag.Badge")
	canvas.Desc(`000000043242565fjinkjh5874rfnjifjhi4j`)
...
func funnel(w http.ResponseWriter, req *http.Request) {
	...
	for size := grid; size < width; size += grid {
		area = (area + 1)%8
		area_f := float64(area) / float64(7)
		fmt.Printf("area: %d, area_f: %f\n", area, area_f)
		r := math.Sqrt(area_f)
		r_i := int(.1 * r * float64(width))
		canvas.Ellipse(size, height/2, r_i, r_i)
	}
	canvas.Gend()
	canvas.End()
}
