...
var (
	AppSpec = tag.FormSpec(amp.AppSpec, "planet")
	//ppTag = amp.TagFromExpr(AppID)
)
...
type appInst struct {
	...
	plSess map[tag.ID]*plSess // mounted planets
	plMu   sync.RWMutex       // plSess mutex
}

// plSess represents a mounted planet (a CellID+AttrID+SI LSM database), allowing it to be accessed, served, and updated.
type plSess struct {
	ctx      task.Context
	app      *appInst
...
func (app *appInst) OnClosing() {
	// TODO: close all mounted planets
}
...
func (app *appInst) homePlanet() (*plSess, error) {
	...
	login := app.Session().Auth()
	if len(login.UserUID) == 0 {
		return nil, amp.ErrCode_PlanetFailure.Errorf("no user logged in")
	}

	var err error
	app.home, err = app.mountPlanet(tag.Nil, login.UserUID)
	if err != nil {
		return nil, err
	}
...
func (app *appInst) ServeRequest(op amp.Requester) (amp.Pin, error) {
	...
	nameLen := strings.IndexByte(url.Path, '/') // amp://planet/<planet-alias>/<cell-alias>
	cellAlias := ""
	if nameLen < 0 {
		nameLen = len(url.Path)
	} else {
		cellAlias = url.Path[nameLen+1:]
	}
	pl, err := app.getPlanet(url.Path[:nameLen], true, true)
	if err != nil {
		return nil, err
	}
	...
	var cellID tag.ID
	if cellAlias != "" {
		cellID, err = pl.resolveCellAlias(cellAlias, true) // TODO: autoIssue == true is a security risk
		if err != nil {
			return nil, err
		}
	}
...
func (app *appInst) getPlanet(findAlias string, autoMount bool, createOK bool) (*plSess, error) {
	...
	if findAlias == "" {
		return nil, amp.ErrCode_PlanetFailure.Errorf("missing planet name")
	}
	planetTag := app.home.GetSymbolID(findAlias, createOK)
	if planetTag.IsNil() {
		return nil, amp.ErrCode_PlanetFailure.Errorf("failed to resolve planet alias '%q'", findAlias)
	}

	app.plMu.RLock()
	pl := app.plSess[planetTag]
	app.plMu.RUnlock()
	if pl != nil {
		return pl, nil
	} else if !autoMount {
...
/*
plApp
	plSess (mounted planet: "home")
        plCell
    plSess (mounted planet)
	    plCell
	    plCell
	...
*/
...
func (pl *plSess) getCell(cellID tag.ID, autoMount bool) (cell *plCell, err error) {
	...
	// Start the cell context as a child of the planet it belongs to
	cell.ctx, err = pl.ctx.StartChild(&task.Task{
		Label: fmt.Sprintf("cell: %0x", cell.ID),
		ID:    cellID,
...
func (pl *plSess) launchPin(cellID tag.ID, op amp.Requester) (amp.Pin, error) {
	...
	if pin.req.PinSync == amp.PinSync_Maintain {
...
func (cell *plCell) commitTx(tx *amp.TxMsg) error {
	...
	for {
		...
		for _, op := range tx.Ops {
			if op.TargetID != cell.ID {
				return amp.ErrCode_MalformedTx.Errorf("target ID mismatch")
			}
...
func (pin *cellPin) execute() error {
	...
	var err error
	switch pin.req.PinSync {

	case amp.PinSync_Maintain:
...
func (pin *cellPin) pushSnapshot() error {
	...
	for itr.Rewind(); itr.Valid(); itr.Next() {
		...
		op := amp.TxOp{
			...
			SI:       tag.From16(key[kIndexOfs:]),
			// Height:   binary.BigEndian.Uint64(key[kHeightOfs:]),
			// Hash:     binary.BigEndian.Uint64(key[kHashOfs:]),
		}

		// append op data (serialized ElemVal) to the tx data store -- no need to unmarshal
...
const (
	...
	kAttrOfs   = 24                   // 16 byte attr index
	kIndexOfs  = 24 + 16              // 16 byte series index
	kHeightOfs = 24 + 16 + 16         // 8 byte height counter
	kHashOfs   = 24 + 16 + 16 + 8     // 8 byte hash      -- TODO
	AttrKeyLen = 24 + 16 + 16 + 8 + 8 // total byte length of key -- TODO
