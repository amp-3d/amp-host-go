...
var LoginAuthAttrID = amp.AttrSpec.With("oauth2.AuthToken").ID
...
func (auth *Config) pushAuthCodeRequest() error {
	...
	return amp.SendMetaAttr(auth.ctx.Session(), tag.ID{}, amp.OpStatus_Synced, std.LaunchURL, val)
}
...
func (auth *Config) OnTokenUpdated(tok *oauth2.Token, saveToken bool) error {
	...
	if saveToken {
		...
		if err == nil {
			auth.ctx.Log().Info(2, "wrote new oauth token")
		} else {
			auth.ctx.Log().Error("error storing token:", err)
		}
	}
	return err
}
